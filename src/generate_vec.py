"""
Compute persona vectors from positive/negative activation CSVs.

For each trait, extracts hidden states from the model for positive and negative
persona examples, then computes difference vectors (pos - neg) at each layer.

Produces three vector types per trait:
...
Output shape: [num_layers+1, hidden_size] (including embedding layer).
"""
...
def save_persona_vector(model_name, pos_path, neg_path, trait, save_dir, threshold=50):
    ...
    print(
        f"Effective samples: {len(persona_pos_effective_prompts)} pos, "
        f"{len(persona_neg_effective_prompts)} neg"
    )
